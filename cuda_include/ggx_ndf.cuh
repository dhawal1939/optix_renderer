...
float sampleNdfPdf(owl::common::vec3f L, owl::common::vec3f H, owl::common::vec3f N, float D) {
	return D * clampDot(N, H, true) / (4 * clampDot(H, L, false));
}



owl::common::vec3f evalBSDF(float alpha, owl::common::vec3f diffuse_color,
	owl::common::vec3f N, owl::common::vec3f H, owl::common::vec3f V, owl::common::vec3f L, owl::common::vec3f pos)
{
	float d = D(N, H, alpha);
